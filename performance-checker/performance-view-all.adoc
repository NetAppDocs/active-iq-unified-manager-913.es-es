...
* MB/s: Para los volúmenes FlexVol, este es el rendimiento del volumen, medido en megabytes por
segundo. Para los volúmenes FlexGroup, esta es la suma de MB/s para todos los volúmenes constituyentes.
* Campos de capacidad: Capacidad libre y total en GIB.
...
A continuación se muestran algunos campos importantes en la vista Performance: All Network interfaces.
...
* QoS Policy Group: El nombre del grupo de políticas de calidad de servicio.
...
* IOPS mínimo absoluto: Para políticas adaptativas, es el valor de IOPS mínimo absoluto que se utiliza como anulación
Cuando el IOPS esperado es inferior a este valor.
...
* Objetos asociados: La cantidad de cargas de trabajo que se asignan al grupo de políticas de calidad de servicio, separadas en volúmenes
Y LUN. Puede hacer clic en el número para desplazarse a una página que proporciona más detalles sobre los volúmenes o LUN seleccionados.
